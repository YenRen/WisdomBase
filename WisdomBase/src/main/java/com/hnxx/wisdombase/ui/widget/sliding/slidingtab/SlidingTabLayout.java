package com.hnxx.wisdombase.ui.widget.sliding.slidingtab;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
     ...
     * tablayout 可用宽度
     */
    private float mTabValidWidth;
    private float mDefaultTabPadding;
    ...
    private int mTitleHorizentalGravity;
    private final int HORIZENTAL_GRAVITY_LEFT = 1;
    private final int HORIZENTAL_GRAVITY_CENTER = 0;
    private final int HORIZENTAL_GRAVITY_RIGHT = 2;
    ...
    private Interpolator mEndInterpolator = new DecelerateInterpolator(1.6f);
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        //水平方向位置
        mTitleHorizentalGravity = ta.getInt(R.styleable.SlidingTabLayout_tl_text_horizontal_gravity,HORIZENTAL_GRAVITY_CENTER);
        mDefaultTabPadding = mTabPadding;
    ...
    private void updateTabStyles() {
        ...
        //Lyg-Update 2018/10/9 计算tab-padding,根据屏幕可用宽度及mTabCount填充，
        int calSumTextWidth = 0;
        for (int i = 0 ; i < mTabCount ; i++){
            if(mTabValidWidth > 0){
                View v = mTabsContainer.getChildAt(i);
                TextView tv_tab_title = v.findViewById(R.id.tv_tab_title);
                textPaint = tv_tab_title.getPaint();
                float tabTextWidth = textPaint.measureText(tv_tab_title.getText().toString());
                calSumTextWidth += tabTextWidth ;
            }
        }
        if(mTabValidWidth > 0 && calSumTextWidth > 0 ){
            if(mTabValidWidth > calSumTextWidth + 2*mTabCount*mDefaultTabPadding){
                //重新计算tabPadding
//                mTabPadding = (mTabValidWidth - calSumTextWidth)/(2*mTabCount);
                mTabPadding = mDefaultTabPadding;
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
            ...
            if (tv_tab_title != null) {
                ...
                if(HORIZENTAL_GRAVITY_LEFT == mTitleHorizentalGravity){
                    tv_tab_title.setPadding(0, 0, (int) mTabPadding*2, 0);
                    if(i == mTabCount-1){
                        //最后一个
                        tv_tab_title.setPadding(0, 0, PhoneInfoTools.dip2px(mContext,4), 0);
                    }
                }else if(HORIZENTAL_GRAVITY_RIGHT == mTitleHorizentalGravity){
                    tv_tab_title.setPadding((int) mTabPadding*2, 0, 0, 0);
                }else{
     ...
     * 专为多维视听新体验页面
     */
    private void updateTabStylesForTwoTable() {
        TextPaint textPaint = null;
        //Lyg-Update 2018/10/9 计算tab-padding,根据屏幕可用宽度及mTabCount填充，
        int calSumTextWidth = 0;
        for (int i = 0 ; i < mTabCount ; i++){
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
    ...
    private void calcIndicatorRect() {
        ...
        if (currentTabView != null) {
            ...
            if (mIndicatorWidth < 0) {   //indicatorWidth小于0时,原jpardogo's PagerSlidingTabStrip
                ...
                float textWidth = mTextPaint.measureText(tab_title.getText().toString());
                if(HORIZENTAL_GRAVITY_LEFT == mTitleHorizentalGravity){
                    indicatorLeft = currentTabView.getLeft()+(textWidth-mIndicatorWidth)/2;
                }else if(HORIZENTAL_GRAVITY_RIGHT == mTitleHorizentalGravity){
                    indicatorLeft = currentTabView.getRight()-(textWidth-mIndicatorWidth)/2 - mIndicatorWidth;
                }
                float indicatorRight = indicatorLeft + mIndicatorWidth;
                if (this.mCurrentTab < mTabCount - 1) {
                    //Lyg-Update 2018/9/26 增加indicator滑动动画
                    //Lyg-Update 2019/7/12 针对最后一个不规则tab，单独处理(即最后一两个之间切换)
                    if(mCurrentTab == mTabCount - 2){
                        indicatorLeft = indicatorLeft + mStartInterpolator.getInterpolation(mCurrentPositionOffset) * (currentTabView.getWidth());
                        indicatorRight = indicatorRight + mEndInterpolator.getInterpolation(mCurrentPositionOffset) * (currentTabView.getWidth());
                    }else{
                        View nextTab = mTabsContainer.getChildAt(this.mCurrentTab + 1);
                        indicatorLeft = indicatorLeft + mStartInterpolator.getInterpolation(mCurrentPositionOffset) * (currentTabView.getWidth() / 2 + nextTab.getWidth() / 2);
                        indicatorRight = indicatorRight + mEndInterpolator.getInterpolation(mCurrentPositionOffset) * (currentTabView.getWidth() / 2 + nextTab.getWidth() / 2);
                    }
                }
    ...
    public float getmTabValidWidth() {
        return mTabValidWidth;
    }

    public void setmTabValidWidth(float tabValidWidth) {
        this.mTabValidWidth = dp2px(tabValidWidth);
    }
    ...
    public void setIndicatorHeightxp(float indicatorHeight) {
    ...
    public void setIndicatorWidthxp(float indicatorWidth) {
    ...
    public void setTextsizeForTwoTable(float textsize) {
        this.mTextsize = sp2px(textsize);
        updateTabStylesForTwoTable();
    }
    ...
    public void setWisdomTwoTableStyle(boolean tabSpaceEqual, int textsize,int indicatorWidth,
                                          int indicatorHight,int indicotorMarginTop){
        this.mTabSpaceEqual = tabSpaceEqual;
        this.mTextsize = sp2px(textsize);
        setIndicatorWidthxp(indicatorWidth);
        setIndicatorHeight(indicatorHight);
        setIndicatorMargin(0,indicotorMarginTop,0,0);
        updateTabStyles();
    }
    public void setWisdomFourTableStyle(boolean tabSpaceEqual, int textsize,
                                          int indicatorHight,int indicotorMarginTop){
        this.mTabSpaceEqual = tabSpaceEqual;
//        this.mTextsize = sp2px(textsize);
        setIndicatorHeight(indicatorHight);
